...
class Continuous2DTrainerV3:
    ...
    def _init_environments(self):
        ...
        self.train_env = VecLowLevelAUV2DEnvV3DirectThrust(num_envs=self.num_envs, **env_kwargs)
        ...
        print("2D V3 Direct Thrust Environment initialized:")
        print(f"  obs_dim={self.obs_dim}, act_dim={self.act_dim} (8 thrusters)")
        print(f"  thrust_scale={self.args.thrust_scale} N")
    ...
    def _init_network(self):
        training_config = self.config["training"]
        # 8维动作用更大网络
        hidden = self.config.get("network", {}).get("feature_extractor", {}).get("hidden_sizes", [256, 128])
...
def main():
    parser = argparse.ArgumentParser(description="Train 2D V3 direct thrust (8 thrusters)")
