...
Offline PPO 预训练高层策略
=========================
Phase 1: 在专家数据集上离线训练，不与环境交互。
高层网络输入 21 维观测，输出 2 维连续动作 (subgoal offset)。
"""
...
class HighLevelActorCritic(nn.Module):
    """高层策略网络: Gaussian actor + value critic"""
    LOG_STD_MIN = -5.0
    LOG_STD_MAX = 0.5
    ...
    def get_dist(self, x):
        h = self.actor_body(x)
        mu = self.mu_head(h)
        log_std = torch.clamp(self.log_std, self.LOG_STD_MIN,
                              self.LOG_STD_MAX)
...
class OfflineDataset:
    """从 .npz 加载专家数据，切分 episode 并计算 GAE"""
    ...
    def __init__(self, path: str, gamma: float = 0.99,
        ...
        self.next_states = torch.tensor(
            raw["next_states"], dtype=torch.float32)
        ...
        # advantages / returns 稍后由 model 的 value 计算
        self.advantages = torch.zeros(self.n)
        self.returns = torch.zeros(self.n)

    def compute_gae(self, model: HighLevelActorCritic):
        """用当前 value 网络估算 GAE"""
        model.eval()
        with torch.no_grad():
            values = model.get_value(
                self.states.to(self.device)).cpu()
            next_values = model.get_value(
                self.next_states.to(self.device)).cpu()
...
class OfflinePPOTrainer:
    def __init__(self, args):
        ...
        self.exp_name = (args.experiment_name
                         or f"offline_ppo_{datetime.now():%Y%m%d_%H%M}")
    ...
    def train_step(self) -> Dict[str, float]:
        """一次 offline PPO 更新 (多 epoch mini-batch)"""
        args = self.args
        # 先重新计算 GAE
        ...
        for _epoch in range(args.n_epochs):
            ...
            if args.target_kl is not None:
                with torch.no_grad():
                    all_new_logp, _ = self.model.evaluate_actions(
                        all_s, all_a)
                    kl = (old_logp.to(self.device)
                          - all_new_logp).mean().item()
    ...
    def train(self):
        ...
        print(f"Offline PPO: {n_iters} iterations, "
        ...
        self.plot_curves()
        print("Offline PPO pretraining done.")
    ...
    def plot_curves(self):
        ...
        plt.suptitle(f"Offline PPO Pretraining — {self.exp_name}")
        plt.tight_layout()
...
def main():
    ...
    trainer = OfflinePPOTrainer(args)
    trainer.train()
