# Flat PE: train pursuer vs scripted evader
import os
import sys
import argparse
import numpy as np
...
def scripted_evader(evader_pos, pursuer_pos, np_random):
    return scripted_evader_run_away(evader_pos, pursuer_pos, 1.0, np_random)

class ActorCritic(nn.Module):
    def __init__(self, obs_dim, n_actions, hidden=(128, 64)):
        super().__init__()
        layers = []
        prev = obs_dim
        for h in hidden:
...
def train(
    ...
    exp_name = experiment_name or f"pe2d_flat_{datetime.now().strftime('%Y%m%d_%H%M')}"
    ...
    for update in range(1, n_updates + 1):
        for step in range(n_steps):
            ...
            for i, e in enumerate(envs):
                state = e.get_state()
                ev_act = scripted_evader(state["evader_pos"], state["pursuer_pos"], e.np_random)
                no, r, term, trunc, info = e.step(int(act_buf[step, i]), evader_action=ev_act)
                if term or trunc:
                    if info.get("caught"):
                        catch_count += 1
                    ep_count += 1
        ...
        if update % 100 == 0:
            ...
            print(f"update {update} | mean_rew {rew_buf.mean():.2f} | catch_rate {sr:.2%}", flush=True)
            torch.save({"model": model.state_dict(), "obs_dim": obs_dim, "n_actions": n_actions}, os.path.join(ckpt_dir, "latest.pth"))
    torch.save({"model": model.state_dict(), "obs_dim": obs_dim, "n_actions": n_actions}, os.path.join(ckpt_dir, "final.pth"))
    print(f"Pursuer saved to {ckpt_dir}", flush=True)
...
def main():
    ...
    p.add_argument("--pursuer-v-max", type=float, default=DEFAULT_PURSUER_V_MAX)
    p.add_argument("--pursuer-a-max", type=float, default=DEFAULT_PURSUER_A_MAX)
    p.add_argument("--evader-v-max", type=float, default=DEFAULT_EVADER_V_MAX)
    p.add_argument("--evader-a-max", type=float, default=DEFAULT_EVADER_A_MAX)
    p.add_argument("--seed", type=int, default=42)
    args = p.parse_args()
    train(experiment_name=args.experiment_name, total_timesteps=args.total_timesteps,
          pursuer_v_max=args.pursuer_v_max, pursuer_a_max=args.pursuer_a_max,
          evader_v_max=args.evader_v_max, evader_a_max=args.evader_a_max, seed=args.seed)
