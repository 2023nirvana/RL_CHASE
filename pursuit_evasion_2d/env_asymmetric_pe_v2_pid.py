"""
二维非对称追逃博弈环境 V2 - RL 输出目标点 + 2D 双环 PID 算 τ
================================================================

与 env_asymmetric_pe_v2_dynamics 的区别：
- 动作语义：RL 输出的是「目标点」（离散 9 个：0=保持，1~8=方向上的目标），不是推力方向
- 追击者每步：target_pos = action_to_target_position_2d(action); τ = PID.compute_thrust(target_pos, pos, vel); dynamics.step(τ)
- 与 low_level 一致：RL 只决策目标，PID 算 τ，动力学积分出 vel/acc；观测含速度、加速度 (22 维)
"""
...
class PursuitEvasion2DAsymmetricEnvV2PID(gym.Env):
    """二维非对称追逃 V2：RL 输出目标点，2D PID 算 τ，动力学积分；观测 22 维（含速度、加速度）。"""
    ...
    def __init__(
        ...
        pid_target_step_length: float = 2.0,
        ...
        self.pid_target_step_length = pid_target_step_length
        ...
        self._p_pid = PIDController2D(
            Kp_pos=2.0,
            Kp_vel=20.0,
            Kd_vel=6.0,
            v_max=pursuer_v_max,
            max_force=float(self._p_dynamics.max_thrust),
            goal_slowdown_radius=1.0,
            goal_slowdown_min_scale=0.2,
        )
    ...
    def _step_pursuer(self, action: int) -> bool:
        target_pos = action_to_target_position_2d(
            action,
            self._p_pos,
            self.world_size,
            step_length=self.pid_target_step_length,
        )
        thrust, _ = self._p_pid.compute_thrust(
            target_pos, self._p_pos.copy(), self._p_vel.copy()
        )
...
def make_easy_env(**kwargs):
    ...
    kw.update(kwargs)
    return PursuitEvasion2DAsymmetricEnvV2PID(**kw)
