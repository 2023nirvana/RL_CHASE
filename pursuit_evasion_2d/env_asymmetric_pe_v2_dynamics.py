"""
二维非对称追逃博弈环境 V2 - 基于 low_level_env 动力学
====================================================

与 env_asymmetric_pe_v2 的区别：
1. 使用 pe_2d_dynamics.Dynamics2D 替代简化的双积分器
2. 动力学方程: M*a = tau - D_linear*v - D_quadratic*|v|*v
3. 加速度由动力学方程计算（含质量、阻尼）
4. 观测空间增加加速度 (22维)

保留与 pe_v2 相同的：奖励结构、逃脱者策略、课程学习、动作空间
"""
...
class PursuitEvasion2DAsymmetricEnvV2Dynamics(gym.Env):
    """二维非对称追逃环境 V2 - 基于 low_level 动力学。观测 22 维。"""
...
def make_easy_env(**kwargs):
    ...
    kw.update(kwargs)
    return PursuitEvasion2DAsymmetricEnvV2Dynamics(**kw)
