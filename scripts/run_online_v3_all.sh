#!/bin/bash
# Online PPO v3: 6 experiments (3 algorithms × 2 param groups) in parallel
set -e
cd "$(dirname "$0")/.."

LOW="low_near/v4_tight_heading_terminal/checkpoints/best.pth"
BPPO_CK="checkpoints/bppo_v2_2m/best.pth"
IQL_CK="checkpoints/iql_v2_2m/best.pth"
TD3BC_CK="checkpoints/td3bc_v4_2m/best.pth"
...
echo "$(date): Starting 6 experiments in parallel..."
mkdir -p logs
...
echo "$(date): All 6 experiments finished!"
