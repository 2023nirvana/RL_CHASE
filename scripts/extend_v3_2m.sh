#!/bin/bash
# Online PPO v3 → 2M steps: 从 500K final checkpoint 继续训练到 2M
set -e
cd "$(dirname "$0")/.."
...
echo "$(date): Extending 6 experiments to 2M steps..."
mkdir -p logs

# BPPO-A
python -u training/online_ppo_finetune.py \
  --low-level-ckpt "$LOW" \
  --resume "checkpoints/v3_bppo_A/final.pth" \
...
python -u training/online_ppo_finetune.py \
  --low-level-ckpt "$LOW" \
  --resume "checkpoints/v3_bppo_B/final.pth" \
...
python -u training/online_ppo_finetune.py \
  --low-level-ckpt "$LOW" \
  --resume "checkpoints/v3_iql_A/final.pth" \
...
python -u training/online_ppo_finetune.py \
  --low-level-ckpt "$LOW" \
  --resume "checkpoints/v3_iql_B/final.pth" \
...
python -u training/online_ppo_finetune.py \
  --low-level-ckpt "$LOW" \
  --resume "checkpoints/v3_td3bc_A/final.pth" \
...
python -u training/online_ppo_finetune.py \
  --low-level-ckpt "$LOW" \
  --resume "checkpoints/v3_td3bc_B/final.pth" \
