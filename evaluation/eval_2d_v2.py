"""
2D V2 模型评估和可视化脚本 (6动作含转向)
==========================================

使用方法：
    cd /root/autodl-tmp/HRL_Control/backup_v0.1_discrete_action/UnderwaterPursuitEvasion
    python evaluation/eval_2d_v2.py --checkpoint checkpoints/2d_v2_dist_5m/best_model.pth --target-distance 5.0
"""
...
class PolicyEvaluatorV2:
    ...
    def __init__(self, checkpoint_path: str, target_distance: float = 5.0, device: str = None):
        self.device = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
        self.target_distance = target_distance
        
        # 创建环境
        self.env = LowLevelAUV2DEnvV2(
    ...
    def run_episode(self, render: bool = False, verbose: bool = True):
        ...
        init_yaw = self.env.inner.dynamics.eta[5]
        trajectory['positions'].append(init_pos.tolist())
        trajectory['yaw_angles'].append(init_yaw)
        ...
        while not done:
            ...
            yaw = self.env.inner.dynamics.eta[5]
            trajectory['positions'].append(pos.tolist())
            trajectory['yaw_angles'].append(yaw)
            trajectory['actions'].append(action)
            trajectory['rewards'].append(reward)
            trajectory['obs'].append(obs.tolist())
            
            if verbose and step % 50 == 0:
                print(f"  Step {step}: pos_error={info.get('pos_error', 0):.3f}m, action={LowLevelAUV2DEnvV2.ACTION_NAMES[action]}")
    ...
    def evaluate(self, n_episodes: int = 10, verbose: bool = True):
        ...
        print(f"\n{'='*60}")
        print(f"Evaluating {n_episodes} episodes (V2, target={self.target_distance}m)...")
        print(f"{'='*60}\n")
    ...
    def plot_trajectory(self, result: dict, save_path: str = None, title_prefix: str = ""):
        ...
        yaw_angles = np.array(trajectory['yaw_angles'])
        ...
        # 左图：2D 轨迹（带航向箭头）
        ax1 = axes[0]
        ax1.plot(positions[:, 0], positions[:, 1], 'b-', linewidth=2, label='Trajectory', alpha=0.7)
        ...
        for i in range(0, len(positions), arrow_interval):
            dx = arrow_length * np.cos(yaw_angles[i])
            dy = arrow_length * np.sin(yaw_angles[i])
            ax1.arrow(positions[i, 0], positions[i, 1], dx, dy, 
                     head_width=0.1, head_length=0.05, fc='orange', ec='orange', alpha=0.6)
        
        # 绘制成功范围（0.2m）
        circle = plt.Circle((target[0], target[1]), 0.2, color='green', fill=False, 
                           linestyle='--', linewidth=2, label='Success Zone (0.2m)')
        ax1.add_patch(circle)
        
        ax1.set_xlabel('X (m)', fontsize=12)
        ax1.set_ylabel('Y (m)', fontsize=12)
        title = f'{title_prefix}2D V2 Trajectory ({"Success" if result["success"] else "Fail"})'
        ...
        # 添加成功线（0.2m）
        ax2_twin.axhline(y=0.2, color='green', linestyle='--', label='Success Threshold')
...
def main():
    parser = argparse.ArgumentParser(description="2D V2 Model Evaluation (6 actions with yaw)")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to checkpoint")
    parser.add_argument("--target-distance", type=float, default=5.0, help="Target distance (m)")
    ...
    # 创建评估器
    evaluator = PolicyEvaluatorV2(args.checkpoint, target_distance=args.target_distance)
